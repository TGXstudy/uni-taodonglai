	...
	<div class="dropdown-item">
		...
		<view class="dropdown-item__selected" @click="changePopup">
			...
			<block v-else>
				<view class="selected__name">
					<view class="selected__img" :class="showClass === 'show'? 'up' : 'down'">
						<text>商品</text>
						<image @tap="godown" :src="(isAll ? '/static/imgs/index/ic_type_bom_false.png' : '/static/imgs/index/ic_type_top_false.png')"
						 mode=""></image>
					</view>
				</view>
		...
		<view class="dropdown-item__content" :style="{top: contentTop + 'px'}" v-if="showList">
			...
			<view :class="['list', showClass]">
				...
				<block v-else>
					<navigator :url="item.url" class="list__option" v-for="(item, index) in list" :key="index" @click="choose(item)">
						<view>{{item.text}}</view>
						<icon v-if="item.value === value" type="success_no_circle" size="12" />
					</navigator>
				</block>
			</view>
			<!-- dropdown-mask -->
