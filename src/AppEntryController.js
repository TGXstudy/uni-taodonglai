	...
	getWeChatOpenId() {
		...
		if (this.$mUtils.checkIsWx()) {
			let url = window.location.href,
				query = this.$mUtils.getRequestParameters(url)
			if (Object.keys(query).length) {
				let code = query.code || ''
				if (!code) {
					let appid = 'wxb37f433fc67146e1',
					...
					return
				} else if (code != this.$store.state.code) {
	...
	handleH5BrowserAddressBarAuth() {
		...
		for (let routeKey in this.$mRoutesConfig) {
			...
			if (route.path == hashPath) {
				if (route.requiresAuth && !this.$u.hasLogin) {
				...
				} else if (route.requireLogin && this.$u.hasLogin) {
