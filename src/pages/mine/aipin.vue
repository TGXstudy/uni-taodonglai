	...
	<view class="jf">
		<view class="top_top">
			<view class="topleft">
				...
				<p class="jff">{{userinfo.love_value}}</p>
	...
	export default {
		...
		jf_mx(){
			...
			this.$api.myLoveValue({
				...
				if (res.code == 1) {
					if (this.page == 1) {
					this.jf_mxlist=res.data.data
					} else {
						this.jf_mxlist = this.jf_mxlist.concat(res.data.data);
					}
					this.nomore = res.data.record.per_page <= this.page;
				} else {
